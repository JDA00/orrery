...
A real-time 3D solar system where the Sun, planets and the Moon are placed by the same
mathematical models astronomers use to predict the sky. Nothing is
hand-animated or keyframed. The positions are calculated, and the
scene follows from the mathematics.
...
The solar system is hard to see. The distances are extremely large, the bodies
can be small and time scales are slow. Every visualization has to choose what
to lie about and these choices shape what's understood.

I wanted to build a visualization that starts from real data and works
backward to something you can look at. Where to compress scale so Mercury
and Neptune can share a frame, lighting a body 4 billion kilometres
from the Sun and what Saturn's rings do when sunlight passes through their
ice particles at a shallow angle all requires aesthetic consideration.
...
More detailed textures, bloom, an accurate starfield, a date selector, additional moons (the Galileans,
Titan) and controller support are on the roadmap.
...
system's centre of mass, not simply from the Sun. This matters because
the Sun itself moves, pulled primarily by Jupiter, orbiting the
barycentre by roughly one solar radius. The Moon's position comes from
ELP82, a dedicated lunar theory.

Both are the same class of models used in professional astronomy. The
project validates its positions against NASA's JPL Horizons ephemeris
service, holding the inner planets to about 10 arcseconds, with relaxed
tolerances for Uranus and Neptune.

The simulation tracks seven astronomical time scales (the same ones
used in spacecraft navigation and GPS) with leap-second handling and
relativistic corrections between Earth-centred and solar-system barycentric time.
Some scale conversions are simplified: UT1 assumes a zero offset from
UTC (accurate to under a second), and the TDB-TT relativistic correction
uses a millisecond-level formula. Both are well within what a visual
simulation needs. "Where is Mars?" depends on both the time system and
the coordinate frame you're using, so the project maintains
...
A real solar system doesn't fit on a screen. The Sun is one million times
Earth's volume. At true scale Neptune would be invisible, a speck
30 AU away. This visualization keeps the astronomical positions real, then
deliberately compresses distances and sizes so the result is
understandable. The scaling is adaptive: inner planets are spread apart
more aggressively than outer ones and moons are enlarged relative to
their parent so you can see them orbit.
...
toward the subsolar point the way real regolith does. Venus uses a
different reflectance law tuned to sulfuric acid cloud tops. Gas giants
...
conservation, plus an opposition surge model that reproduces the
brightening seen when a body is lit head-on, a phenomenon caused by
shadow-hiding among surface particles.

Saturn's rings are the most involved piece of rendering. Sunlight hitting
...
while the tenuous C ring lets most light through. When the rings pass
...
Requires Java 17 (the Gradle wrapper handles the toolchain).

Textures are not stored in the repository. They are packaged as a tarball and attached to a GitHub Release.
On first run, the `fetchTextures` Gradle task downloads the tarball, verifies its
SHA-256 checksum and extracts the files into `src/main/resources/textures/`.
